        ...
        <div class="row g-2 align-items-center mb-4">
            ...
            <div class="col-md-auto ms-auto mt-4 mt-md-0 d-print-none">
                <div class="d-flex flex-wrap flex-md-nowrap">
                    <input id="keyword" value="<?= $filter_key ?>" type="search" class="form-control d-inline-block w-9 me-md-3 me-0 mb-md-0 mb-2" placeholder="Search kode / nama produk">
                    <div class="w-100 mb-md-0 mb-2">
                        <select class="form-select" id="kategori">
                            <option value="0" <?= ($filter_kat == '0') ? 'selected' : '' ?>>Semua Kategori</option>
                            <?php foreach ($data_kategori as $kategori) : ?>
                                <option value=<?= $kategori['id'] ?> <?= ($filter_kat == $kategori['id']) ? 'selected' : '' ?>><?= $kategori['nama'] ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    ...
                    <?php if (in_groups('pusat')) : ?>
                        <a href="<?= base_url("/produk/new") ?>" class="btn btn-primary ms-auto ms-md-3 mt-3 mt-md-0">
                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered">
                                ...
                                <?php else : ?>
                                    <tr class="text-center">
                                        <td colspan="12">Belum ada data produk.</td>
                                    </tr>
                                <?php endif; ?>
                            </table>
                        </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body text-center py-4">
                ...
                <div class="text-muted">
                    <p>Apakah Anda yakin ingin menghapus data produk <strong><span id="modal-product-name" class="text-danger">ini</span></strong>?</p>
                    <p>Data terkait produk ini juga akan terhapus dan data yang sudah dihapus TIDAK DAPAT DIKEMBALIKAN.</p>
                </div>
            </div>
...
<script>
    $(document).ready(function() {
        $('#keyword').on('keyup', function() {
            $.get('cari-produk?keyword=' + $('#keyword').val() + '&kategori=' + $('#kategori').val() + '&supplier=' + $('#supplier').val(), function(data) {
        ...
        $('body').on('click', '.btn-hapus', function(e) {
            ...
            $('#modal-product-name').html(nama);
            $('#form-hapus').attr('action', '/produk/' + id);
        });
    })
</script>
<?= $this->endSection() ?>